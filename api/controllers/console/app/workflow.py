...
pipeline_variable_model = console_ns.model(
    "PipelineVariable", pipeline_variable_fields)

# Workflow model with nested dependencies
workflow_fields_copy = workflow_fields.copy()
workflow_fields_copy["created_by"] = fields.Nested(
    simple_account_model, attribute="created_by_account")
workflow_fields_copy["updated_by"] = fields.Nested(
    simple_account_model, attribute="updated_by_account", allow_null=True
)
workflow_fields_copy["conversation_variables"] = fields.List(
    fields.Nested(conversation_variable_model))
workflow_fields_copy["rag_pipeline_variables"] = fields.List(
    fields.Nested(pipeline_variable_model))
...
workflow_pagination_fields_copy["items"] = fields.List(
    fields.Nested(workflow_model), attribute="items")
workflow_pagination_model = console_ns.model(
    "WorkflowPagination", workflow_pagination_fields_copy)
...
try:
    ...
    simple_end_user_model = console_ns.model(
        "SimpleEndUser", simple_end_user_fields)

try:
    workflow_run_node_execution_model = console_ns.models.get(
        "WorkflowRunNodeExecution")
except (KeyError, AttributeError):
    workflow_run_node_execution_model = console_ns.model(
        "WorkflowRunNodeExecution", workflow_run_node_execution_fields)
...
class AdvancedChatDraftRunIterationNodeApi(Resource):
    ...
    def post(self, app_model: App, node_id: str):
        ...
        parser = reqparse.RequestParser().add_argument(
            "inputs", type=dict, location="json")
        args = parser.parse_args()
...
parser_block = reqparse.RequestParser().add_argument(
...
class WorkflowFeaturesApi(Resource):
    ...
    def post(self, app_model: App):
        ...
        parser = reqparse.RequestParser().add_argument(
            "features", type=dict, required=True, location="json")
        args = parser.parse_args()
        ...
        workflow_service.update_draft_workflow_features(
            app_model=app_model, features=features, account=current_user)
...
class DraftWorkflowTriggerNodeApi(Resource):
    ...
    def post(self, app_model: App, node_id: str):
        ...
        node_type: NodeType = draft_workflow.get_node_type_from_node_config(
            node_config)
        event: TriggerDebugEvent | None = None
        ...
        files = _parse_file(draft_workflow, raw_files if isinstance(
            raw_files, list) else None)
        try:
            ...
            return jsonable_encoder(
                {"status": "error",
                    "error": "An unexpected error occurred while running the node."}
            ), 400
...
class WorkflowOnlineUsersApi(Resource):
    ...
    def get(self):
        parser = reqparse.RequestParser().add_argument(
            "workflow_ids", type=str, required=True, location="args")
        args = parser.parse_args()

        workflow_ids = [workflow_id.strip()
                        for workflow_id in args["workflow_ids"].split(",")]

        results = []
        for workflow_id in workflow_ids:
            users_json = redis_client.hgetall(
